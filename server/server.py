    ...
    if command["action"]=="autorization":
        ...
        if command["token"]+"\n" in all_token:
            ...
            for build in all_buildings:
                cursor.execute(f"INSERT INTO {build} (id_user, count, buildings_start_time,new_buildings_count) VALUES (%s, %s, %s, %s)", (user_id,0,time_now,0))
            for army in all_army:
                cursor.execute(f"INSERT INTO {build} (id_user, count, army_start_time,new_army_count) VALUES (%s, %s, %s, %s)", (user_id,0,time_now,0))        
            connection.commit()
